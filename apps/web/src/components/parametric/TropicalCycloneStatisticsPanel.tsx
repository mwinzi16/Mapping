...
export default function TropicalCycloneStatisticsPanel({ embedded = false }: TropicalCycloneStatisticsPanelProps) {
  const { boxes, statistics, selectedBoxId, isLoadingStats, calculateAllStatistics, selectBox } = useParametricStore()
  ...
  const renderContent = () => (
    <div className={embedded ? 'px-4 pb-4' : 'p-4 max-h-80 overflow-y-auto'}>
      {/* Header with Analyze All button - only show when not embedded */}
      {!embedded && (
        <div className="flex items-center justify-between mb-4">
          ...
          {hasBoxes && (
            ...
            >
              {isLoadingStats ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Analyze All'}
            </button>
          )}
        </div>
      )}
  ...
  return embedded ? renderContent() : <div className="max-h-80 overflow-y-auto">{renderContent()}</div>
}
