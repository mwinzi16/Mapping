...
class EarthquakeTriggerCriteria(BaseModel):
    ...
    min_magnitude: Optional[float] = Field(None, ge=0, le=10, description="Minimum magnitude")
    max_depth_km: Optional[float] = Field(None, ge=0, description="Maximum depth in kilometers (shallower = more damaging)")
    min_depth_km: Optional[float] = Field(None, ge=0, description="Minimum depth in kilometers")
...
class EarthquakeBoundingBox(BaseModel):
    ...
    trigger: Optional[EarthquakeTriggerCriteria] = Field(None, description="Trigger criteria for this box")
...
class EarthquakeBoxStatistics(BaseModel):
    ...
    qualifying_earthquakes: int = Field(0, description="Earthquakes meeting trigger criteria")
    years_analyzed: int
    annual_frequency: float
    qualifying_annual_frequency: float = Field(0.0, description="Annual frequency of qualifying events")
    magnitude_distribution: Dict[str, int] = Field(default_factory=dict, description="Magnitude range -> count")
    depth_distribution: Dict[str, int] = Field(default_factory=dict, description="Depth range -> count")
    monthly_distribution: Dict[int, int] = Field(default_factory=dict, description="Month -> count")
