...
class TriggerCriteria(BaseModel):
    ...
    min_category: Optional[int] = Field(
        None, ge=0, le=5, description="Minimum Saffir-Simpson category (0=TS, 1-5)"
    )
    min_wind_knots: Optional[int] = Field(
        None, ge=0, description="Minimum wind speed in knots"
    )
    max_pressure_mb: Optional[int] = Field(
        None, ge=800, le=1100, description="Maximum central pressure in millibars"
    )

    def matches(
        self, category: int, wind_knots: int, pressure_mb: Optional[int]
    ) -> bool:
        """Check if a hurricane point matches the trigger criteria.

        Args:
            category: Saffir-Simpson category.
            wind_knots: Maximum sustained wind in knots.
            pressure_mb: Central pressure in millibars.

        Returns:
            ``True`` when the data point meets all criteria.
        """
...
class BoundingBox(BaseModel):
    ...
    trigger: Optional[TriggerCriteria] = Field(
...
class BoxStatistics(BaseModel):
    ...
    qualifying_hurricanes: int = Field(
        0, description="Hurricanes meeting trigger criteria"
    )
    years_analyzed: int
